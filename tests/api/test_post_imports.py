...
async def test_post_imports_generated(api_client, sync_connection):
    fake_cloud = FakeCloud()
...
async def test_post_imports_case1(api_client, sync_connection):
    """
    update from d1/d2/d3/d4 to d1/d3/(d2, d4). Here d2 is just a name, not number or location.
    One of sophisticated cases that impossible for regular user to do in one update through cloud GUI"""
    fake_cloud = FakeCloud()
